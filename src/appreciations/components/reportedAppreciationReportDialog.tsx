...
import { useLazyReportedAppreciationReportQuery } from '../apiSlice';
 ...
 *   Q1 ends May 31  → exclusive end June 1
 *   Q2 ends Aug 31  → exclusive end Sep 1
 *   Q3 ends Nov 30  → exclusive end Dec 1
 *   Q4 ends Feb 28/29 of startYear+1 → exclusive end Mar 1 next year
 */
...
export default function ReportedAppreciationReportDialog(props: IProps) {
  ...
  const [fetchReport, { isFetching }] = useLazyReportedAppreciationReportQuery();
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (result.data) {
        ...
        a.href = url;
        a.download = `reported_appreciation_report_Q${quarter}_${year}.xlsx`;
  ...
  return (
    <React.Fragment>
      <Dialog open={props.open} onClose={handleClose} fullWidth maxWidth="sm">
        <DialogTitle>Download Reported Appreciations Report</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            Select a year and quarter to download the Reported Appreciations report.
          </DialogContentText>
          <FormControl fullWidth variant="standard" sx={{ mb: 2 }}>
            <InputLabel id="rep-appr-report-year-label">Year</InputLabel>
            <Select
              labelId="rep-appr-report-year-label"
              id="rep-appr-report-year-select"
          ...
          <FormControl fullWidth variant="standard" sx={{ mb: 2 }}>
            <InputLabel id="rep-appr-report-quarter-label">Quarter</InputLabel>
            <Select
              labelId="rep-appr-report-quarter-label"
              id="rep-appr-report-quarter-select"
